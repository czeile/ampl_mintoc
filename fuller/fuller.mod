...
var f {k in X,o in 0..no,i in I} = (
	...
	else if (k==2 && o==0) then 1
	else if (k==1 && o==1) then 0
	else if (k==2 && o==1) then -2
);

var lagrange{i in IC} = (x[1,i])^2+(x[1,nt] - 0.01)^2 + (x[2,nt] - 0.0)^2;

var re{k in RE} = (
	if      (k==1) then x[1,0] - 0.01
	else if (k==2) then x[2,0] - 0.0
	else if (k==3) then x[1,nt] - 0.01
	else if (k==4) then x[2,nt] - 0.0
);
