...
#	if      (k==1 && o==0) then curktr*x[1,i]*( ( curBase/x[3,i] )^curgamma  - 1.0) - ( curslope*log(1.0+x[4,i]*V1*MM) )*x[1,i]
	if      (k==1 && o==0) then curktr*x[1,i]*( (1-(curslope*log(1.0+x[4,i]*V1*MM)))*( curBase/x[3,i] )^curgamma  - 1.0)
	else if (k==2 && o==0) then curktr*( x[1,i]- x[2,i] )
	else if (k==3 && o==0) then curktr*x[2,i] - kcirc*x[3,i]
